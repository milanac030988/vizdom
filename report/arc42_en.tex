...
\midrule
\endhead
App under test & inbound & Screenshot (BGR image) via a capture strategy; input events via an actuator strategy. \\
...
(distributed gRPC), ADR-018 (capture), ADR-019 (actuator).
...
\begin{table}[H]
  ...
  \caption{OmniParser element-type accuracy on matched actionable controls, before
  and after the role-mapping heuristic (synthetic test set).}
  \label{tab:arc42typeacc}
  \begin{tabular}{@{}lccccc@{}}
    \toprule
    Mapping & button & input\_field & checkbox & icon & actionable \\
    \midrule
    Before (text/icon only) & 0.00 & 0.00 & 0.00 & 1.00$^\dagger$ & 0.09 \\
    After (caption + geometry) & \textbf{0.89} & \textbf{0.99} & \textbf{0.55} & 0.89 & \textbf{0.87} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\midrule
\endhead
\code{visual\_dom.cv} & Pipeline orchestration; OCR; symbol/screen detection; the pluggable \emph{detectors} subpackage; the SLM advisor. \\
\code{visual\_dom.capture} & CapturePort: strategy base, registry, built-ins (windows/linux/android/camera), gRPC client, user plugins. \\
\code{visual\_dom.actuator} & ActuatorPort: strategy base, registry, built-ins (desktop/android), gRPC client, user plugins. \\
\code{visual\_dom.hierarchy} & Coarse hierarchy builder; optional LLM refiner. \\
\code{visual\_dom.compiler} & DOM compiler: roles, flags, locators, labels; schema. \\
\code{visual\_dom.rpc} & gRPC servers for detector, capture, actuator. \\
\code{visual\_dom} (shared) & Reading-order, logging, schema. \\
...
The \code{detectors} subpackage holds a \code{DetectorBackend} interface, a registry
factory, and the UIED/YOLO/OmniParser/gRPC backends; the pipeline additionally owns
merge/split/NMS, smart-merge, reading-order, and the SLM-review stages (Ch.~6).
...
D-02 & Registry/discovery code duplicated across 3 ports & Low / -- & Factor a shared plugin-registry base. \\
\bottomrule
\end{longtable}
