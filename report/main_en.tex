...
Table~\ref{tab:typeacc} reports the effect. The old mapping scored actionable
type-accuracy of only 0.09 --- correct solely on icons, which it obtained trivially
by labelling \emph{everything} \code{icon} (hence its inability to identify anything
else). The improved mapping raises actionable accuracy to \textbf{0.87}: buttons
0.89, input fields 0.99, and \emph{icons and checkboxes recovered from 1.00/0.00 and
0.00 to 0.89 and 0.55}. OmniParser's DOM is now directly usable by the test layer
across all interactive roles.

\begin{table}[H]
  \centering
  \caption{Element-type classification accuracy on matched actionable controls,
  before and after the improved mapping (OmniParser, synthetic test set).
  Localisation (Table~\ref{tab:pertype}) is unaffected --- this measures \emph{type}
  only. $^\dagger$The old mapping labelled all non-text as \code{icon}, so icons were
  ...
  \begin{tabular}{lccccc}
    ...
    After (caption + width/aspect) & \textbf{0.89} & \textbf{0.99} & \textbf{0.55} & 0.89 & \textbf{0.87} \\
    \bottomrule
  \end{tabular}
\end{table}
