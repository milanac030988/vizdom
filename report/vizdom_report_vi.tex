...
Dice score, với acceptance threshold riêng cho từng ký hiệu. Trên ảnh chụp thật
của một ứng dụng máy tính, cơ chế này đọc đúng 14 trong 15 control được kiểm tra;
glyph tổ hợp ``$\pm$'' còn lại được từ chối an toàn thay vì bị nhận dạng sai. Kết
quả được đưa vào \code{text} --- vì ký hiệu là nội dung được đọc trực tiếp --- nên
một phím toán tử vẫn có thể được điều khiển bằng \code{text="+"} ngay cả khi
caption của nó bị nhiễu. Tập ký hiệu được nhận dạng và mức độ nghiêm ngặt có thể
cấu hình trong section \code{symbols}.
...
tìm được một kết quả đủ tin cậy:
