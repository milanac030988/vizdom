...
``$\div$'' và ``$-$'' không match được gì. Giai đoạn này hiện đưa \emph{mọi}
candidate nhị phân hóa (cả hai polarity và adaptive threshold) qua template
matching rồi chọn kết quả tốt nhất trên toàn bộ tập candidate. Artefact dạng vòng
không khớp template nào, nên polarity đúng được chọn theo bằng chứng từ phép so khớp
thay vì bằng heuristic. Hệ thống đồng thời bổ sung size gate cho dấu ``.'' và các
template dùng font đậm.

Trên cả hai theme, cơ chế hiện đọc đúng \textbf{20 trên 20} control được kiểm tra;
glyph tổ hợp ``$\pm$'' vẫn được từ chối an toàn thay vì bị nhận dạng sai. Kết quả
được đưa vào \code{text} --- vì ký hiệu là nội dung được đọc trực tiếp --- nên một
...
Action phân giải locator thành phần tử, chuyển center sang tọa độ chuẩn hóa rồi ủy quyền cho actuator. Nhờ vậy, cùng một bài kiểm thử có thể chạy cục bộ, qua gRPC hoặc với cánh tay robot mà không cần thay đổi. Hình~\ref{fig:seq} mô tả tương tác đầu-cuối của một chu kỳ từ ảnh chụp màn hình đến DOM.
