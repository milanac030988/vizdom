...
tế. Báo cáo này được nộp theo hình thức đồ án ứng dụng Method-3: đóng góp chính là
...
Với tính chất là một đồ án ứng dụng Method-3, các đóng góp chủ yếu thuộc về kỹ thuật:
\begin{enumerate}[leftmargin=1.6em]
...
\paragraph{Một phân rã theo hướng microservice.} Các port không chỉ là đường nối nội
bộ: mỗi port \emph{detector}, \emph{capture} và \emph{actuator} là một ranh giới dịch
vụ hạng nhất, có hợp đồng protobuf được đánh phiên bản (\code{protos/*.proto}) và một
máy chủ gRPC độc lập, nên bất kỳ port nào cũng có thể được triển khai, mở rộng và nâng
cấp như một microservice độc lập. Định hướng này mang lại lợi ích microservice đúng
nơi bài toán cần: \emph{mở rộng độc lập} (một detector warm-GPU phục vụ nhiều client
mỏng), \emph{đặt vị trí không đồng nhất} (capture trên thiết bị, detection trên máy
GPU, actuation tại SUT hoặc bộ điều khiển cánh tay robot), \emph{cô lập lỗi và tài
nguyên} (một detector bận hay hỏng không làm nghẽn capture), và \emph{thay thế đa
ngôn ngữ} (một dịch vụ có thể được viết lại bằng bất kỳ ngôn ngữ nào sau cùng một hợp
đồng). Do đó hệ thống theo hướng microservice ngay từ thiết kế --- các ranh giới dịch
vụ được thiết kế sẵn chứ không chắp vá --- trong khi, theo điểm kế tiếp, chính các
dịch vụ đó gộp lại thành một process khi việc đặt chung là đủ, nên phân tán được áp
dụng dần thay vì phải trả giá trước.
...
\subsection{Xây dựng và tăng cường dữ liệu}
Không có benchmark công khai nào nhắm đúng bối cảnh của VizDOM --- các GUI tự vẽ và
quan sát qua camera kèm ground truth có cấu trúc --- nên dữ liệu được chính dự án sinh
ra, theo ba lớp bổ trợ đánh đổi giữa quy mô, sự đa dạng và tính hiện thực của kênh thu.

\paragraph{Sinh bằng chương trình (benchmark có nhãn).} Các mẫu UI tham số hóa
(login, dashboard, \dots) được kết xuất qua các chủ đề sáng/tối và nhiều độ phân giải,
mỗi lần sinh ra một ảnh chụp màn hình \emph{kèm theo} ground truth chính xác (bounds,
\code{visual\_type}, text, containment) theo đúng schema pipeline sinh ra. Điều này
cho nhãn không nhiễu ở quy mô lớn --- chính là tập chia 167/44 dùng ở trên --- mà
không cần annotation thủ công, nên nó đóng vai trò benchmark tái lập được.

\paragraph{Bố cục do LLM thiết kế (đa dạng).} Các mẫu viết tay đơn điệu về thị giác,
nên một mô hình ngôn ngữ lớn (Claude) được dùng như một \emph{nhà thiết kế UI}: khi
được nhắc cho một lớp ứng dụng (ví dụ màn hình thông tin-giải trí trên xe hơi), nó đề
xuất một bố cục cụ thể rồi một renderer biến thành ảnh chụp màn hình kèm ground truth
tương ứng. Điều này bơm vào sự đa dạng cấu trúc --- các cụm điều khiển bất thường và
widget đặc thù theo lĩnh vực --- mà mẫu cố định không phủ được, với chi phí gán nhãn
không đáng kể.

\paragraph{Tăng cường kiểu camera (bền vững theo miền).} Vì một động lực trung tâm là
đọc màn hình \emph{qua camera}, các ảnh sạch được tăng cường để mô phỏng kênh đó: một
biến đổi phối cảnh (góc nhìn lệch trục) cùng lóa sáng, mờ và nhiễu cảm biến, theo một
hoặc nhiều biến thể mỗi ảnh. Nhãn ground truth được giữ nguyên --- chính xác cho các
hiệu ứng quang học, và hợp lệ cho trường hợp phối cảnh vì VizDOM trước tiên nắn chỉnh
vùng màn hình phát hiện được về góc nhìn chính diện trước khi phát hiện. Tập này rèn
luyện đường nắn màn hình và trực tiếp thử thách điểm khác biệt không-cần-cây-trợ-năng
của VizDOM.

Cùng nhau, ba lớp này kết hợp quy mô gán nhãn rẻ (sinh chương trình), đa dạng cấu trúc
(LLM thiết kế) và tính hiện thực của kênh thu (tăng cường): tập tổng hợp là benchmark
tái lập được, còn các lớp LLM và camera mở rộng độ phủ hướng tới điều kiện triển khai
thực. Do đó các điểm số tuyệt đối trong phần này được báo cáo trên tập tổng hợp sạch;
các lớp LLM và camera là công cụ phát triển/kiểm thử độ bền chứ không phải benchmark
được chấm điểm, và một tập có nhãn từ thực tế lớn hơn vẫn là công việc tương lai.
