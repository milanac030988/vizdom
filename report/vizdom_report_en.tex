...
thresholds. On a real calculator screenshot this reads 14 of 15 probed controls
correctly (the remaining composite ``$\pm$'' glyph is safely rejected rather than
misread), and its output feeds \code{text} --- a symbol is a literal read --- so an
operator key is drivable as \code{text="+"} even when its caption is noise. The
recognised set and strictness are configurable (\code{symbols} section).
