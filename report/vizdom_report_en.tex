...
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{amssymb}      % \checkmark in the defect-comparison table
\usepackage{caption}
\usepackage{enumitem}
\usepackage{xcolor}
...
\paragraph{A sixth difficulty specific to testing: naming non-text elements.}
Beyond the five properties above --- which concern \emph{finding} elements ---
automation additionally requires \emph{naming} them, and a large share of GUI
controls carry no readable text at all: toolbar icons, window controls, operator
keys, glyph-only buttons. Detection localises such a control perfectly and still
leaves the test author with nothing to write in a locator. This is a distinct
problem, not a corollary of detection accuracy, and it has three unattractive
default answers. Position is brittle (it breaks on any relayout, the very failure
image-based tools suffer from). OCR returns nothing, because there is no text ---
or worse, returns noise from a decorative glyph. A caption model returns a
\emph{prediction}, which we measured to be unstable: two captures of the same
screen differing only by imperceptible re-encoding noise ($\leq$7/255 per channel)
flipped an icon's caption from ``Maximize'' to ``Page'' (Section~\ref{sec:eval}).
VizDOM therefore treats non-text semantics as a first-class concern with a layered
answer --- literal glyph reading where the shape is a known symbol, caption-derived
labels kept strictly separate from read text, canonical names for recognised
controls, and description-based grounding as the escape hatch --- described in
Section~\ref{sec:impl} and evaluated in Section~\ref{sec:eval}.
...
\paragraph{A microservice-oriented decomposition.} The ports are not merely internal
seams: each of \emph{detector}, \emph{capture}, and \emph{actuator} is a first-class
service boundary with a versioned protobuf contract (\code{protos/*.proto}) and a
standalone gRPC server, so any port can be deployed, scaled, and upgraded as an
independent microservice. This orientation buys the microservice benefits exactly
where the problem needs them: \emph{independent scaling} (one warm-GPU detector
serves many thin clients), \emph{heterogeneous placement} (capture on the device,
detection on a GPU host, actuation at the SUT or a robot-arm controller),
\emph{fault and resource isolation} (a busy or crashed detector does not stall
capture), and \emph{polyglot substitution} (a service can be reimplemented in any
language behind the same contract). The system is thus microservice-oriented by
construction --- the service boundaries are designed in, not retrofitted --- while,
per the next point, those same services collapse into one process when co-location
suffices, so distribution is adopted incrementally rather than paid for up front.
...
\subsection{Literal text, semantic labels, and symbol reading}
Two refinements separate what is \emph{read} from what is \emph{inferred}. First,
the DOM distinguishes \code{text} --- the literal pixel read (OCR) --- from
\code{label} --- the element's semantic name, resolved with priority \emph{nearby
associated label $>$ detector caption $>$ own text}. The distinction matters
because caption models are predictions, not reads: two visually identical captures
differing only by imperceptible re-encoding noise ($\leq$7/255 per channel) flipped
an icon caption from ``Maximize'' to ``Page''. Keeping captions out of \code{text}
makes locators depend only on what is actually on screen.

Second, glyph-only controls (operator keys \code{+ - = $\times$ $\div$}, window
controls) defeat both OCR and, previously, a rule-based symbol detector whose
projection heuristics failed on real key glyphs and mistook a hollow maximize
square for ``+''. The redesigned stage reads such glyphs by \emph{template
matching}: the glyph is cropped to its own bounding box (scale-normalised),
hollow outline shapes are rejected structurally, and the candidate is matched
against font-rendered templates by Dice score with per-symbol acceptance
thresholds. A second capture --- the same application rendered with thicker
glyphs --- exposed a subtler failure: the \emph{binarisation choice} was wrong.
An adaptive-threshold \emph{ring artifact} (the glyph appearing as a hole inside a
blob) scored best on the density heuristic that selected among binarisations, so a
fat ``$\times$'' matched the filled-dot template while ``$\div$'' and ``$-$''
matched nothing. The stage therefore now scores \emph{every} binarisation
candidate (both polarities plus adaptive), with an added size gate for ``.'' and
bold font templates.

Scaling the evaluation up forced a third, deeper revision: a similarity score
alone cannot say ``this is not a symbol at all''. On a 130-case labelled set ---
four real renderings of the same application (two keypad layouts, 100\% and 125\%
display scaling, dark and light theme), including every key that must be
\emph{rejected} --- the template matcher scored 51/130: canonicalised to a fixed
patch, a word key (``mod''), a backspace icon, or a digit becomes a bar-like
smear that matches ``$-$'' or ``$=$'' above threshold, and a \emph{degraded}
binarisation that loses components (the dots of ``$\div$'', the second bar of
``$=$'') outscored honest readings from better candidates. The final design
classifies by \emph{connected-component structure} --- ``$\div$'' must be a wide
bar with one dot above and one below, ``$=$'' exactly two stacked solid bars,
``$+$'' a centred cross with empty corners --- with templates demoted to
supporting evidence, and ranks binarisation candidates by \emph{specificity}:
the reading that explains more components wins, because degradation can only
lose structure, never invent it. A glyph fitting no structure is rejected. The
result is \textbf{130 of 130} across all four renderings, kept as a data-driven
regression test; on the synthetic set the same change removed three false
positives and recovered two previously-missed ``\%'' glyphs. Its output feeds
\code{text} --- a symbol is a literal read --- so an
operator key is drivable as \code{text="+"} even when its caption is noise; a
confident read additionally assigns a \emph{canonical} label
(``$\div$''$\rightarrow$Divide, ``$\times$''$\rightarrow$Multiply), replacing
look-alike caption noise on operator keys while title-bar controls keep the
captions that are their real names. The recognised set and strictness are
configurable (\code{symbols} section).
...
\end{itemize}
Locators support \code{id=}, \code{text=}, \code{hint=}, \code{role=}, spatial
relations (\code{within=}, \code{right\_of=}, \code{below=}, \dots) and
\code{desc=} (Section~\ref{sec:desc}). Several locators in one string are
combined with \emph{and}; a \code{||} separator instead expresses an ordered
\emph{fallback chain}, tried left to right until one alternative resolves to
exactly one element:
...
\noindent An alternative is abandoned both when it matches nothing and when it is
\emph{ambiguous} --- an ambiguous locator is as unusable as a missing one --- and a
failed chain reports every attempt with its reason, so chains remain debuggable.
Because alternatives are ordered, the author controls cost: a deterministic
\code{text=} lookup is free and runs first, while \code{desc=} (which may invoke a
model) is reached only after it has failed. When a later alternative wins, the
library warns and names the alternative that failed --- a signal that the primary
locator has gone stale, and the measurable input to the self-healing locators
listed as future work (ADR-024).

Actions resolve a locator to an element, convert its center to normalized
coordinates, and delegate to the actuator, so the same test runs locally, over
gRPC, or against a robot arm without change.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth]{Locator_Resolution.png}
  \caption{Resolving a locator to exactly one element. Alternatives separated by
  \code{||} are tried in order; an alternative is abandoned when it matches
  \emph{nothing} or is \emph{ambiguous}, a \code{desc=} in the chain may first
  arbitrate among ambiguous candidates (ADR-024), a lone description escalates
  through the grounding tiers (Figure~\ref{fig:descgrounding}), and a chain that
  fails reports every attempt with its reason.}
...
\begin{figure}[H]
  ...
  \caption{End-to-end interaction for one action. The costly perception step runs
  \emph{once} per screen; each subsequent locator is a lookup in the cached DOM and
  a click never re-captures. The element centre is normalised through the capture
  frame's geometry before the actuator maps it into device pixels
  (Section~\ref{sec:impl}), and reading a value back uses the region-level recap
  rather than a new detector pass.}
  \label{fig:locatorseq}
\end{figure}

\subsubsection*{Targeting the right application}
A less obvious correctness requirement follows from the two design choices above:
the system captures the \emph{whole screen} and actuates by \emph{absolute
coordinate}. Both are silently wrong whenever the application under test is not the
foreground window. A grab taken while another window overlaps it produces a Visual
DOM of the wrong application, and a click at coordinates that were correct for that
screenshot lands on whatever window now occupies the point --- while typed text goes
to whatever holds keyboard focus. Nothing in the DOM can reveal the mistake: the
coordinates were consistent with the screenshot, and the screenshot was consistent
with its moment.

The system therefore treats ``bring the application to the front'' as an explicit
capability of both driven ports (\code{focus\_target()}, ADR-021), implemented per
platform: the Win32 escalation sequence (restore, borrow the foreground thread's
input queue, synthetic modifier tap, temporary top-most) on Windows,
\code{wmctrl}/\code{xdotool} on X11, and \code{am start} against the resumed
activity on Android. Two properties make it trustworthy rather than merely
convenient. First, success is \emph{verified} --- the strategy re-reads the actual
foreground window afterwards --- because the operating system may legitimately
refuse the request (Windows' foreground lock), and an optimistic ``true'' would
convert that refusal into an unexplained wrong screenshot. Second, focus is
performed \emph{where the screen is}: in a distributed run it travels as its own
\code{Focus} RPC to the capture and actuator services, since a client on another
host cannot raise a window it does not own. Ambiguity is a failure, not a guess: a
window title matching several windows is reported with its candidates instead of
driving an arbitrary one. Exposure is deliberately two-tiered --- an explicit
keyword that fails the test, and an opt-in configuration flag that raises the target
before every capture and only warns --- because raising a window is itself an action
that can dismiss transient UI, so it must never be implied by a read. Figure~\ref{fig:seq} shows the
end-to-end interaction for a single screenshot-to-DOM cycle.
...
\subsection{Description-based location: tiered grounding over the DOM}
\label{sec:desc}
Property locators are deterministic but brittle exactly where perception is
weakest: an icon whose caption is unstable, or a key whose OCR is noise, is
unfindable by \code{text=}. A \code{desc=} locator therefore lets the tester state
\emph{intent} --- \code{desc="settings button"} --- and imports the grounder
paradigm (Section~\ref{sec:dombenefits}) into the parser system, with the DOM kept
as the source of truth. Resolution escalates through configurable tiers, each
cheaper and more deterministic than the next, stopping at the first confident hit:
(Figure~\ref{fig:descgrounding})
(i) a \emph{lexical} tier (model-free) scores token/synonym overlap against
\code{label}/\code{text}/\code{hint} plus role words, accepting only with a score
floor \emph{and} a margin over the runner-up --- ties escalate rather than guess;
(ii) an \emph{SLM-over-DOM} tier gives a small text model (temperature 0) a
compact element table and validates the returned id against the DOM (hallucinated
ids are rejected); (iii) an opt-in \emph{VLM} tier uses Set-of-Mark prompting ---
candidate boxes are numbered on the screenshot and the model picks a number, so
its answer is a DOM element \emph{by construction}, avoiding fragile coordinate
regression. Two findings shaped the design. Positional words (``top right'') are
enforced \emph{geometrically} before any model: a 3B model confidently
misidentified a top-left element as ``top right'', so position is never delegated.
And determinism is a policy, not an accident: the tier chain is set in the session
configuration (CI can pin the lexical tier only), resolutions are cached per
screen, and every hit records which tier answered --- making the ``parse once,
ground only the long tail'' economics measurable. On a real calculator session the
chain resolved 5/5 probe descriptions (paraphrases via the SLM tier) and correctly
rejected a nonsense description with the candidate list (ADR-022).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.62\linewidth]{Desc_Grounding.png}
  \caption{Tiered grounding of a \code{desc=} locator (ADR-022). Each tier is
  cheaper and more deterministic than the next; a tie \emph{escalates} rather
  than guesses, positional words are enforced geometrically before any model,
  a hallucinated id is rejected against the DOM, and the VLM tier answers with
  a DOM element by construction (Set-of-Mark over the stored screenshot). The
  same resolver, restricted to the ambiguous candidates, arbitrates
  duplicate-property matches (ADR-024).}
...
itself, in three complementary layers that trade off scale, diversity, and channel
realism.

\paragraph{Programmatic synthesis (the labelled benchmark).} Parameterised UI
templates (login, dashboard, \dots) are rendered across light/dark themes and
resolutions, each emitting a screenshot \emph{together with} exact ground truth
(element bounds, \code{visual\_type}, text, containment) in the pipeline's own
schema. This yields noise-free labels at scale --- the 167/44 train/test split used
above --- with no manual annotation, which is why it serves as the reproducible
benchmark.

\paragraph{LLM-designed layouts (diversity).} Hand-written templates are visually
monotonous, so a large language model (Claude) is used as a \emph{UI designer}:
prompted for an application class (e.g.\ a car-infotainment screen), it proposes a
concrete layout that a renderer turns into a screenshot with matching ground truth.
This injects structural variety --- unusual control groupings and domain-specific
widgets --- that fixed templates cannot cover, at negligible labelling cost.

\paragraph{Camera-style augmentation (domain robustness).} Because a central
motivation is reading a screen \emph{through a camera}, clean screenshots are
augmented to imitate that channel: a perspective warp (off-axis viewpoint) plus
glare, blur, and sensor noise, in one or more variants per image. The ground-truth
labels carry over --- exact for the photometric effects, and valid for the
perspective case because VizDOM first rectifies the detected screen region back to a
frontal view before detection. This set exercises the screen-rectification path and
directly stresses VizDOM's no-accessibility-tree differentiator.

Together these layers combine cheaply-labelled scale (synthesis), structural
diversity (LLM design), and channel realism (augmentation): the synthetic split is
the reproducible benchmark, while the LLM and camera layers broaden coverage toward
real deployment conditions. Absolute scores in this section are therefore reported on
the clean synthetic split; the LLM and camera layers are development/robustness aids
rather than the scored benchmark, and a larger real-world labelled set remains future
work.
...
Figure~\ref{fig:robotsuite} shows the shipped end-to-end demo as it actually
executes: a Robot Framework suite driving the Windows Calculator with all three
ports as gRPC services. It also makes the cost structure concrete --- the suite
performs \textbf{two} detector calls in total (one in the suite setup, one before
the assertion that inspects the display content), and the second test case adds
none at all, because its clicks resolve against the Visual DOM the first test
already built. This is the reuse property claimed in
Section~\ref{sec:dombenefits}, observed on a real run rather than argued.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{Robot_Suite_Sequence.png}
  \caption{The runnable demo suite end to end
  (\code{examples/windows\_calculator\_demo}). Services are started once and
  outlive many runs; the suite setup raises the application under test and builds
  the DOM once; each subsequent action is a cached-DOM lookup plus one normalised
  \code{Tap}; and reading a value back uses the region-level recap instead of a
  further detector pass.}
  \label{fig:robotsuite}
\end{figure}
...
\subsection{Qualitative evaluation on a real application}
\label{sec:realapp}
The synthetic benchmark measures localisation and typing, but not how the stages
interact on a real, dense enterprise screen. To probe that, one such application
($980\times562$, a client-management console) was analysed with four
detector/OCR combinations and the defects classified by kind
(Table~\ref{tab:combos}). This is a \emph{qualitative} study --- there is no ground
truth for this screen --- so it is reported as observed defect classes rather than
scores.

\begin{table}[H]
  \centering\footnotesize
  \caption{Observed defect classes per detector/OCR combination on one real
  application. ``$\checkmark$'' = no defect of that kind observed.}
  \label{tab:combos}
  \begin{tabular}{@{}p{4.5cm}p{2.2cm}p{2.2cm}p{2.4cm}p{2.2cm}@{}}
    \toprule
    Combination & Icons & Tile buttons & Small status text & Bottom links \\
    \midrule
    UIED + EasyOCR & some missed & $\checkmark$ & $\checkmark$ & merged into one \\
    OmniParser + EasyOCR \emph{(before)} & $\checkmark$ & split in 2--3 & rows missing & $\checkmark$ \\
    OmniParser + Tesseract & $\checkmark$ & $\checkmark$ & rows missing & $\checkmark$ \\
    \textbf{OmniParser + EasyOCR \emph{(after)}} & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
    \bottomrule
  \end{tabular}
\end{table}

Two of the three defect classes turned out to be \emph{our} bugs, visible only by
comparing combinations. First, small status rows (``Active'', ``Inactive'',
``Low Protection'') were absent although the classical baseline read them: the OCR
ensemble (Section~\ref{sec:impl}) only \emph{gap-filled}, adding our
upscaling-OCR boxes where OmniParser had none. Where OmniParser's own OCR had
produced a garbage read in a very small box (``\textasciitilde ctive'' for
``Active'', 8\,px tall), our better read was discarded as a duplicate and the tiny
box was then removed by the pipeline's minimum-size filter --- deleting the row
entirely. The ensemble is now a \emph{prefer-external union}: an external box
\emph{replaces} the OmniParser box it overlaps, since the upscaling read is the
higher-quality one by construction. Second, tile buttons whose caption is two words
(``Check Now'') were split into halves, because the rule-based splitter divides any
control containing two or more OCR boxes --- and EasyOCR emits \emph{word} boxes
where Tesseract emits one line box, which is exactly why the defect appeared with
one engine and not the other. The splitter now refuses to divide a control the
detector marked as a single interactable, and otherwise requires a real gap
($\geq 1.5\times$ the median text height) between texts. A third, unrelated
observation --- a configured OCR engine that is not installed silently degrading a
run to the detector's own OCR --- is now a fast failure with an install hint.

After these fixes, OmniParser + EasyOCR is defect-free on all four classes for
this screen, making it the unambiguous recommended combination, with no regression
on the synthetic benchmark or the calculator probes. Two lessons generalise:
per-type synthetic metrics do not surface stage-interaction bugs, and keeping both
the detector and the OCR engine swappable was what made these bugs findable at all.
