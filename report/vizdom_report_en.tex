...
candidate (both polarities plus adaptive) through template matching and keeps the
best overall --- ring artifacts match no template, so the correct polarity wins on
evidence rather than on a heuristic --- with an added size gate for ``.'' and bold
font templates. Across both themes it now reads \textbf{20 of 20} probed controls
correctly (the composite ``$\pm$'' glyph is still safely rejected rather than
misread). Its output feeds \code{text} --- a symbol is a literal read --- so an
...
gRPC, or against a robot arm without change. Figure~\ref{fig:seq} shows the
