...
(gRPC phân tán), ADR-018 (capture) và ADR-019 (actuator). Cách phân rã này chủ đích
theo \emph{hướng microservice}: mỗi port là một ranh giới dịch vụ hạng nhất với hợp
đồng protobuf được đánh phiên bản và một máy chủ độc lập, mang lại mở rộng độc lập
(một detector warm-GPU cho nhiều client), đặt vị trí không đồng nhất và cô lập lỗi
--- trong khi chính các dịch vụ đó gộp về một process khi đặt chung là đủ, nên phân
tán được áp dụng dần. Hai quyết định sau đó hoàn thiện bề mặt phía client: một
\emph{cấu hình phiên} JSON duy nhất điều khiển mọi giai đoạn từ \code{connect()} hay
từ khóa \code{Connect} của Robot Framework (ADR-020), và bộ định vị \code{desc=}
phân giải mô tả phần tử bằng ngôn ngữ tự nhiên qua một chuỗi định vị phân tầng ---
khớp từ vựng tất định trước, rồi một mô hình ngôn ngữ nhỏ trên DOM, tùy chọn một mô
hình thị giác qua Set-of-Mark --- với từ chỉ vị trí được thực thi bằng hình học và
cache theo màn hình (ADR-022).
...
\midrule
\endhead
\code{visual\_dom.cv} & Điều phối pipeline; OCR; phát hiện symbol/screen; subpackage \emph{detectors} có thể thay thế; SLM advisor. \\
\code{visual\_dom.capture} & CapturePort: strategy base, registry, các implementation tích hợp sẵn (windows/linux/android/camera), gRPC client và plugin của người dùng. \\
\code{visual\_dom.actuator} & ActuatorPort: strategy base, registry, các implementation tích hợp sẵn (desktop/android), gRPC client và plugin của người dùng. \\
\code{visual\_dom.hierarchy} & Coarse hierarchy builder; LLM refiner tùy chọn. \\
\code{visual\_dom.compiler} & DOM compiler: role, flag, locator, label và schema. \\
\code{visual\_dom.rpc} & Các gRPC server cho detector, capture và actuator. \\
\code{visual\_dom} (dùng chung) & Reading order, logging và schema. \\
...
Subpackage \code{detectors} chứa interface \code{DetectorBackend}, registry factory và
các backend UIED/YOLO/OmniParser/gRPC; pipeline còn sở hữu các bước merge/split/NMS,
smart-merge, reading-order và SLM review (Ch.~6).
...
020 & Cấu hình phiên phía client (\code{connect} + một JSON cho mọi giai đoạn) & Đã chấp thuận \\
022 & Bộ định vị theo mô tả (\code{desc=}) qua định vị phân tầng (từ vựng $\rightarrow$ SLM-trên-DOM $\rightarrow$ VLM Set-of-Mark) & Đã chấp thuận \\
\bottomrule
\end{longtable}
